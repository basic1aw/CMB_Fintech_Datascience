...
tags_test = pd.read_csv('X:/CS2/招商银行Fintech/B榜/评分数据集b/评分数据集_tag_b.csv')
...
# 对交易行为和APP行为两个特征标进行feature engineering
beh_test = pd.read_csv('X:/CS2/招商银行Fintech/B榜/评分数据集b/评分数据集_beh_b.csv')
...
trd_test = pd.read_csv('X:/CS2/招商银行Fintech/B榜/评分数据集b/评分数据集_trd_b.csv')
...
for p in [p_train,p_test]:
    for page in page_set:
            ...
            def check(label):
                ...
                # 逐个检查每个用户是否访问过该页面
            p.loc[:,page] = p.loc[:,'page_no'].apply(check).astype('int')
...
#提取原始特征的统计量作为最终的特征，把每个用户的交易量均值以及标准差提取出来
...
# 现根据可能存在关系的特征对含缺失值的特征进行条件填充
for m in [train,test]:
    fillnull(m,'age','perm_crd_lmt_cd','trx_std')
...
new = ['cur_debit_cnt','cur_credit_cnt','id']
cols = test.drop(columns=new).columns.values

# 对job_year进行离散化，同时对其余的连续特征进行归一化
for i in [train,test]:
...
submission_lr = pd.DataFrame({'id':test['id'].values,'flag_predicted':pred_lr})
sub_lr = submission_lr.sort_values(by='id')
path_lr = 'X:/CS2/招商银行Fintech/SUB/LR/LR_b_one_hot.csv'
sub_lr.to_csv(path_lr,header=False,index=False)
...
path_rfc = 'X:/CS2/招商银行Fintech/SUB/xgboost/b_xgb_one_hot.csv'
